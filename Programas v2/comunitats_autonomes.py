...
try: # Creacion de excepcion
   with open (pathFitxer,'r') as fitxer: # Abrir archivo .DAT en modo lectura
       for linea in fitxer: # Bucle para leer linea a linea archivo .DAT
            if linea[11:13] == '99' and linea[9:11] !='99': # Solo tomamos codigo 99 si datos nivel total provincial
                llista = []
                llista.append(linea[14:64].strip()) # Nombre ambito territorial
                llista.append(linea[9:11]) #Codigo comunidad autonoma
                insert_comunitats_autonomes = ('INSERT INTO comunitats_autonomes (nom, codi_ine) VALUES (%s, %s)')
